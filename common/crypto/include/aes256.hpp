#pragma once

#include "debug.hpp"
#include "crypto_utils.hpp"

#include <string>
#include <array>
#include <cstddef>
#include <iostream>
#include <random>
#include <string>
#include <array>
#include <cstddef>
#include <iomanip>
#include <stdexcept>
#include <boost/multiprecision/cpp_int.hpp>

namespace common {
namespace crypto {

class AES256 {
public:
	static constexpr size_t Nk = 8;            // number of 32-bit words in the key (256/32 = 8)
	static constexpr size_t Nb = 4;            // number of columns in state (fixed in AES)
	static constexpr size_t Nr = 14;           // number of rounds (14 for AES-256)

	static constexpr size_t key_length = 256;  // key length in bits
	static constexpr size_t block_size = 128;  // block size in bits (fixed in AES)
	static constexpr size_t state_size = 4 * Nb;

	static constexpr uint8_t sbox[256] = {
		
	};

	static constexpr uint8_t inv_sbox[256] = {
		
	};

	static constexpr uint32_t round_const[10] = {
		0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
		0x20000000, 0x40000000, 0x80000000, 0x1b000000, 0x36000000
	};

public:
	using cpp_int = boost::multiprecision::number<boost::multiprecision::cpp_int_backend<>, boost::multiprecision::et_off>;
	AES256();
	AES256(const std::array<uint8_t, key_length/8>& key);

	std::string aes256_encrypt(const std::string& input);
	std::string aes256_decrypt(const std::string& input);
	// ???
	void set_key(const std::array<uint8_t, key_length/8> key);
	std::string get_key() const;
private:
	std::array<uint8_t, 32> _key;

// temp public intead of protected
public:
	uint8_t gmul(uint8_t a, uint8_t b);
	std::array<uint8_t, 4> rot_word(const std::array<uint8_t, 4>& word);
	std::array<uint8_t, 4> sub_word(const std::array<uint8_t, 4>& word);
	std::array<uint8_t, 4 * Nb * (Nr + 1)> key_expansion();
	std::array<uint8_t, state_size> sub_bytes(const std::array<uint8_t, state_size>& state);
	std::array<uint8_t, state_size> inv_sub_bytes(const std::array<uint8_t, state_size>& state);
	std::array<uint8_t, state_size> shift_rows(const std::array<uint8_t, state_size>& state);
	std::array<uint8_t, state_size> inv_shift_rows(const std::array<uint8_t, state_size>& state);
	std::array<uint8_t, state_size> mix_columns(const std::array<uint8_t, state_size>& state);
	std::array<uint8_t, state_size> inv_mix_columns(const std::array<uint8_t, state_size>& state);
	std::array<uint8_t, state_size> add_round_key(
		const std::array<uint8_t, state_size>& state,
		const std::array<uint8_t, 4 * Nb * (Nr + 1)>& w,
		int round);
	std::vector<uint8_t> pkcs7_pad(const std::string& input);
	std::string pkcs7_unpad(const std::vector<uint8_t>& input);
	std::array<uint8_t, state_size> encrypt(const std::array<uint8_t, state_size>& input);
	std::array<uint8_t, state_size> decrypt(const std::array<uint8_t, state_size>& input);
};

} // namespace crypto
} // namespace common